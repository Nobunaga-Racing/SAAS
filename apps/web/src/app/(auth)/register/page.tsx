...
const schema = z.object({
  tenantName: z.string().min(2, "Nom de l'entreprise requis"),
  tenantSlug: z
    .string()
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    setServerError('')
    try {
      const res = await api.post<{ accessToken: string; user: unknown }>('/auth/register', data)
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-xl font-bold text-gray-900">
              Créer votre compte <span className="text-indigo-600">SaaSGestion</span>
            </h1>
            <p className="text-gray-500 text-sm mt-1">{"Commencez gratuitement dès aujourd'hui"}</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="border-t border-gray-100 pt-4">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Votre compte</p>
              <div className="grid grid-cols-2 gap-4">
                <Input
                  label="Prénom"
                  placeholder="Jean"
