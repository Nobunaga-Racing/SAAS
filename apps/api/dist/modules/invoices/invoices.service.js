...
exports.generateDepositInvoice = generateDepositInvoice;
exports.markOverdueInvoices = markOverdueInvoices;
exports.formatInvoice = formatInvoice;
exports.getInvoiceWithSettings = getInvoiceWithSettings;
...
function calculateLine(line) {
    ...
    if (line.discountType && line.discountValue) {
        lineDiscountAmt =
            line.discountType === invoices_types_1.DiscountType.PERCENT
 ...
 */
function calculateTotals(calculatedLines, discountType, discountValue, depositAmount = 0, amountPaid = 0) {
    const subtotalHt = round(calculatedLines.reduce((s, l) => s + l.lineTotalHt, 0));
    let discountAmount = 0;
    if (discountType && discountValue) {
        discountAmount =
            discountType === invoices_types_1.DiscountType.PERCENT
...
// ─── Numérotation ─────────────────────────────────────────────────────────────
async function generateNumber(tenantId, type) {
    const year = new Date().getFullYear();
    const prefix = type === client_1.InvoiceType.DEPOSIT_INVOICE ? 'ACPT'
        : type === client_1.InvoiceType.CREDIT_NOTE ? 'AV'
            : 'FAC';
...
// ─── CRUD ─────────────────────────────────────────────────────────────────────
async function createInvoice(dto, tenantId, userId) {
    const type = dto.type ?? client_1.InvoiceType.INVOICE;
    return prisma.$transaction(async (tx) => {
        const number = await generateNumber(tenantId, type);
        const calculatedLines = dto.lines.map(calculateLine);
        const totals = calculateTotals(calculatedLines, dto.discountType, dto.discountValue, dto.depositAmount ?? 0, 0);
        return tx.invoice.create({
            data: {
                ...
                depositAmount: new library_1.Decimal(dto.depositAmount ?? 0),
                ...
                amountDue: new library_1.Decimal(totals.amountDue),
                createdBy: userId,
                lines: {
                ...
                vatSummary: {
                    create: totals.vatSummary.map((v) => ({
                        vatRate: new library_1.Decimal(v.vatRate),
                        baseHt: new library_1.Decimal(v.baseHt),
                        vatAmount: new library_1.Decimal(v.vatAmount),
                    })),
                },
            },
...
async function updateInvoice(invoiceId, dto, tenantId) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const totals = calculateTotals(linesToUse, (dto.discountType ?? invoice.discountType), dto.discountValue ?? (invoice.discountValue ? Number(invoice.discountValue) : undefined), dto.depositAmount ?? Number(invoice.depositAmount), Number(invoice.amountPaid));
        if (dto.lines) {
        ...
        return tx.invoice.update({
            ...
            data: {
                ...
                ...(dto.discountValue !== undefined && { discountValue: dto.discountValue ? new library_1.Decimal(dto.discountValue) : null }),
                ...(dto.depositAmount !== undefined && { depositAmount: new library_1.Decimal(dto.depositAmount) }),
                subtotalHt: new library_1.Decimal(totals.subtotalHt),
                discountAmount: new library_1.Decimal(totals.discountAmount),
                totalHt: new library_1.Decimal(totals.totalHt),
                totalVat: new library_1.Decimal(totals.totalVat),
                ...
                ...(dto.lines && {
                    ...
                    vatSummary: {
                        create: totals.vatSummary.map((v) => ({
                            vatRate: new library_1.Decimal(v.vatRate), baseHt: new library_1.Decimal(v.baseHt), vatAmount: new library_1.Decimal(v.vatAmount),
                        })),
                    },
                }),
            },
...
async function findAll(tenantId, filters) {
    ...
    if (overdue) {
        where.status = { in: [client_1.InvoiceStatus.SENT, client_1.InvoiceStatus.PARTIAL] };
...
async function findById(invoiceId, tenantId) {
...
async function sendInvoice(invoiceId, tenantId) {
    const invoice = await findByIdOrThrow(invoiceId, tenantId);
    if (invoice.status !== client_1.InvoiceStatus.DRAFT)
        throw new Error('Seul un brouillon peut être envoyé');
    return prisma.invoice.update({ where: { id: invoiceId }, data: { status: client_1.InvoiceStatus.SENT, sentAt: new Date() } });
}
async function cancelInvoice(invoiceId, tenantId) {
    const invoice = await findByIdOrThrow(invoiceId, tenantId);
    if (['PAID', 'CANCELLED'].includes(invoice.status)) {
        throw new Error('Cette facture ne peut pas être annulée');
    }
    return prisma.invoice.update({ where: { id: invoiceId }, data: { status: client_1.InvoiceStatus.CANCELLED } });
}
async function createCreditNote(invoiceId, tenantId, userId) {
    const invoice = await findByIdOrThrow(invoiceId, tenantId);
    if (invoice.type !== client_1.InvoiceType.INVOICE)
        throw new Error('Seule une facture peut générer un avoir');
    return prisma.$transaction(async (tx) => {
        const number = await generateNumber(tenantId, client_1.InvoiceType.CREDIT_NOTE);
        return tx.invoice.create({
            data: {
                tenantId,
                clientId: invoice.clientId,
                number,
                type: client_1.InvoiceType.CREDIT_NOTE,
                status: client_1.InvoiceStatus.DRAFT,
                issueDate: new Date(),
                dueDate: new Date(),
...
// ─── Paiements ────────────────────────────────────────────────────────────────
async function recordPayment(invoiceId, dto, tenantId) {
    const invoice = await findByIdOrThrow(invoiceId, tenantId);
    if (['PAID', 'CANCELLED'].includes(invoice.status)) {
...
async function deletePayment(paymentId, invoiceId, tenantId) {
    return prisma.$transaction(async (tx) => {
        ...
        const newStatus = newAmountPaid === 0 ? client_1.InvoiceStatus.SENT : client_1.InvoiceStatus.PARTIAL;
        return tx.invoice.update({
            where: { id: invoiceId },
            data: { amountPaid: new library_1.Decimal(newAmountPaid), amountDue: new library_1.Decimal(newAmountDue), status: newStatus, paidAt: null },
        });
    });
}
// ─── Acomptes ─────────────────────────────────────────────────────────────────
async function createDeposit(invoiceId, dto, tenantId) {
    const invoice = await findByIdOrThrow(invoiceId, tenantId);
    const amountResolved = dto.amountType === invoices_types_1.DiscountType.PERCENT
    ...
    const deposit = await prisma.deposit.create({
        data: {
            ...
            amountValue: new library_1.Decimal(dto.amountValue),
            amountResolved: new library_1.Decimal(amountResolved),
    ...
    await prisma.invoice.update({
        where: { id: invoiceId },
        data: { depositAmount: new library_1.Decimal(newDepositAmount), amountDue: new library_1.Decimal(newAmountDue) },
    });
    return deposit;
}
async function generateDepositInvoice(depositId, tenantId, userId) {
    ...
    if (deposit.status !== client_1.DepositStatus.PENDING)
        throw new Error("Facture d'acompte déjà émise");
    return prisma.$transaction(async (tx) => {
        const number = await generateNumber(tenantId, client_1.InvoiceType.DEPOSIT_INVOICE);
        const vatRate = 20;
        ...
        const depInvoice = await tx.invoice.create({
            data: {
                ...
                number,
                type: client_1.InvoiceType.DEPOSIT_INVOICE,
                status: client_1.InvoiceStatus.DRAFT,
                issueDate: new Date(),
                dueDate: deposit.dueDate,
                subject: `Acompte sur facture ${deposit.invoice.number}`,
                subtotalHt: new library_1.Decimal(amountHt),
                discountAmount: new library_1.Decimal(0),
                totalHt: new library_1.Decimal(amountHt),
                totalVat: new library_1.Decimal(vatAmount),
                totalTtc: new library_1.Decimal(Number(deposit.amountResolved)),
                amountPaid: new library_1.Decimal(0),
                amountDue: new library_1.Decimal(Number(deposit.amountResolved)),
                depositAmount: new library_1.Decimal(0),
                createdBy: userId,
                lines: {
                    create: [{
                            description: `Acompte ${Number(deposit.amountValue)}${deposit.amountType === 'PERCENT' ? '%' : '€'} — ${deposit.invoice.number}`,
                            quantity: new library_1.Decimal(1),
                            unitPrice: new library_1.Decimal(amountHt),
                            vatRate: new library_1.Decimal(vatRate),
                            lineBaseHt: new library_1.Decimal(amountHt),
                            lineDiscountAmt: new library_1.Decimal(0),
                            lineTotalHt: new library_1.Decimal(amountHt),
                            lineVatAmount: new library_1.Decimal(vatAmount),
                            lineTotalTtc: new library_1.Decimal(Number(deposit.amountResolved)),
                            position: 0,
                        }],
                },
                vatSummary: {
                    create: [{ vatRate: new library_1.Decimal(vatRate), baseHt: new library_1.Decimal(amountHt), vatAmount: new library_1.Decimal(vatAmount) }],
                },
            },
        });
        await tx.deposit.update({
            where: { id: depositId },
            data: { status: client_1.DepositStatus.INVOICED, depositInvoiceId: depInvoice.id, depositInvoiceNumber: depInvoice.number },
        });
        return depInvoice;
    });
}
...
async function markOverdueInvoices() {
    const result = await prisma.invoice.updateMany({
        where: { status: { in: [client_1.InvoiceStatus.SENT, client_1.InvoiceStatus.PARTIAL] }, dueDate: { lt: new Date() } },
        data: { status: client_1.InvoiceStatus.OVERDUE },
    });
    return result.count;
}
// ─── Helpers ──────────────────────────────────────────────────────────────────
async function findByIdOrThrow(invoiceId, tenantId) {
...
function formatInvoice(inv) {
    return {
        ...
        vatSummary: (inv.vatSummary ?? []).map((v) => ({
...
async function getInvoiceWithSettings(invoiceId, tenantId) {
    const invoice = await findByIdOrThrow(invoiceId, tenantId);
    ...
    return { invoice: formatInvoice(invoice), settings: (tenant ?? fallback) };
}
