...
function generateInvoicePdf(data) {
    return new Promise((resolve, reject) => {
        ...
        // ─── Company logo ────────────────────────────────────────────────────────
        if (settings.logoUrl) {
            const logoPath = path_1.default.join(process.cwd(), settings.logoUrl);
            if (fs_1.default.existsSync(logoPath)) {
                try {
                    doc.image(logoPath, 50, 45, { fit: [80, 60] });
                }
                catch { /* skip */ }
            }
        }
        ...
        if (settings.addressLine1) {
            doc.text(settings.addressLine1, cx, cy, { width: 195, align: 'right' });
            cy += 12;
        }
        if (settings.zipCode || settings.city) {
            doc.text(`${settings.zipCode ?? ''} ${settings.city ?? ''}`.trim(), cx, cy, { width: 195, align: 'right' });
            cy += 12;
        }
        if (settings.phone) {
            doc.text(settings.phone, cx, cy, { width: 195, align: 'right' });
            cy += 12;
        }
        if (settings.siret) {
            doc.text(`SIRET : ${settings.siret}`, cx, cy, { width: 195, align: 'right' });
            cy += 11;
        }
        ...
        const tLw = 115;
        const tVw = 80;
        const addRow = (label, value, bold = false, color = '#374151') => {
            doc.fontSize(9)
